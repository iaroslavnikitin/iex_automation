Feature: Two member Simple A


    @NJ_Regression @PA_Regression @NV_Regression @Eligibility @SimpleA
    Scenario Outline: A married couple filing a joint tax returs, And Application filer’s spouse must be included on the application, but is not seeking coverage herself as the application filer is over age 65.
        ...
        Then I click on OK On Information About Medicare Pop up
        ...
        Examples:
            | fileName                 | role       |
            | ssap_twoMem_SimpleA.json | individual |
