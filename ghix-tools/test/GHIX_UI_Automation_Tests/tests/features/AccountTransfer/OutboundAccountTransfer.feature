Feature: Outbound Account Transfer

  @NJ_Regression @PA_Regression @NV_Regression
  Scenario Outline: Outbound AT
    ...
    Then Update CMR Household Information
    Then Get SSAP Application Id From DB
    Then Update Medicaid Outbound Status to Quarantine And Wait
    Then Verify MEDICAID_OUTBOUND Table Status TRANSMITTED
    #    Then Verify GI_WS_PAYLOAD Table Status SUCCESS
    Then Verify MEDICAID_INBOUND Table Status COMPLETE
    Then Print Out Json Results

    Examples:
      | fileName                  | role       |
      | ssapMedicaidEligible.json | individual |
