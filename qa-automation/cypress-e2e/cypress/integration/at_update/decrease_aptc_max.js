...
inputHouseholds.forEach( file => {
    describe(`Decrease APTC max amount for ${file}`, function() {
        ...
        it(`Post updated AT for ${file}`, function () {
            let hh = cy.h
            cy.log("APTC max ammount before decrease: " + hh.people[0].aptcMaximumAmount)
            for (let i = 0; i < hh.people.length; i++){
                hh.people[i].aptcMaximumAmount = hh.people[i].aptcMaximumAmount - 25
            }
            cy.log("APTC max ammount after decrease: " + hh.people[0].aptcMaximumAmount)
            cy.h = hh
