...
const l2_csr_username = 'cic_40_cypress_l2_csr@yopmail.com'
const l2_csr_pass = 'Ghix123#'
const test_case = 'CIC40'

inputHouseholds.forEach( file => {
    describe(`${test_case}: Enrolls into Medical/Dental then Disenrolls from all through My Eligibility History page - ${file}`, function() {
        ...
        it(`${test_case}: Disenrolls from all through My Eligibility History page - ${file}`, function () {
            //will fail due to
            cy.disenrollAllMyEligibility();
            cy.h.appStatus = cy.h.oep ? "at_created" : "disenroll_all_marriage"
        })
        it(`${test_case}: Verify dashboard`, function () {
