...
const inputHouseholds = ["regression/APTC_2HH_ADDRESS"]
...
const l2_csr_username = 'mnuat760_18_cypress_l2_csr@yopmail.com'
const l2_csr_pass = 'Ghix123#'
const test_case = 'mnuat760_18'

inputHouseholds.forEach(file => {
    describe(`${test_case}: 2 MEMBER WITH IN QEP AND ADD MEMBER IN SEP AND ADDRESS CHANGE IN SEP for ${file}`, function () {
...
//        })
        ...
        it(`${test_case}: Adding a member for ${file}`, function () {
            cy.addMember()
        })
...
//        })
        ...
        it(`${test_case}: Make sure env is SEP and enroll new member and change plan (manually) - ${file}`, function () {
                    cy.debug()
        })
         ...
         it(`${test_case}: Address change for ${file}`, function () {
             cy.h.address = {
                 "ns2:LocationStreet": {
                     "ns2:StreetFullText": "123 Fake Street"
                 },
