...
const l2_csr_username = 'mnuat760_8_cypress_l2_csr@yopmail.com'
const l2_csr_pass = 'Ghix123#'
const test_case = 'mnuat760_8'

inputHouseholds.forEach(file => {
    describe(`${test_case}: 2 MEMBER WITH IN QEP AND ADD CHILD IN SEP AND ADDRESS CHANGE IN SEP for ${file}`, function () {
...
//        })
        ...
        it(`${test_case}: Make sure env is SEP and aptc change (manually) - ${file}`, function () {
                    cy.debug()
        })

                it(`${test_case}: Change APTC amount  for ${file}`, function () {
                            cy.aptcMaxAmountChange(150) // increase by 150
                })
