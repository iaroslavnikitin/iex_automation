...
const l2_csr_username = 'cic_39_cypress_l2_csr@yopmail.com'
const l2_csr_pass = 'Ghix123#'
const test_case = 'CIC39'

inputHouseholds.forEach( file => {
    describe(`${test_case}: Enroll medical/dental then Disenroll from Dental via My Eligibility Page for ${file}`, function() {
        ...
        it(`${test_case}: Disenrolls from Dental through My Eligibility History page - ${file}`, function () {
            ...
            cy.h.appStatus = cy.h.oep ? "med_full_enroll" : "enroll_all_disenroll_dent_birth"
        })
        it(`${test_case}: Verify dashboard`, function () {
            cy.verifyDashboard(cy.h)
        })
