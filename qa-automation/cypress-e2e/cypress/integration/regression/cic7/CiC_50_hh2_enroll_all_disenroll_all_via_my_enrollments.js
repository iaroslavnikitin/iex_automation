...
const l2_csr_username = 'cic_50_cypress_l2_csr@yopmail.com'
const l2_csr_pass = 'Ghix123#'
const test_case = 'CIC50'

inputHouseholds.forEach( file => {
    describe(`${test_case}: Enroll all Disenrolls all through My Enrollments page - ${file}`, function() {
        ...
        it(`${test_case}: Disenrolls from Health through My Enrollments page - ${file}`, function () {
            cy.disenrollHealthMyEnrollments();
            cy.h.appStatus = cy.h.oep ? "enroll_dent" : "disenroll_all_marriage"
        })
        it(`${test_case}: Verify dashboard`, function () {
            cy.verifyDashboard(cy.h)
        })
        it(`${test_case}: Disenrolls from Dental through My Enrollments page - ${file}`, function () {
            cy.disenrollDentalMyEnrollments();
            if (cy.h.oep) cy.h.appStatus = "at_created"
        })
        it(`${test_case}: Verify dashboard`, function () {
            cy.verifyDashboard(cy.h)
        })
